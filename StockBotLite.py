...
shares=[0,0,0,0]
tradecounter=0
print("Welcome to an Your Automated Trader (Lite edition)")
        ...
        while True:
            if clock.is_open:
                ...
                if counter%2==0:
                    ...
                    print(balance)
                    if(tradecounter<3):
                        print("Have not reached daily buy limit")
                    else:
                        print("Have reached daily buy limit")
                hold[0]+=1
                hold[1]+=1
                hold[2]+=1
                hold[3]+=1
                #Hold Variable is not used in StockBotLite, due to infrequent nature of trades
            else:
...
def analyze(symbol, hist, index):
    ...
    if positions[index]!=0:
        ...
        current_time = datetime.datetime.now(pytz.timezone('US/Eastern'))
        if current_time.hour==15 and (current_time.minute>53):
            end=True
        else:
            power=float(account.cash)
            if (float(hist[size-3])>float(hist[size-4]) and float(hist[size-1])>float(hist[size-2]) and float(hist[size-2])>float(hist[size-3]) and power>currPrice and tradecounter<4):
